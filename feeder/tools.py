...
def temporal_slice(data_numpy, step):
    ...
    return data_numpy.reshape(C, T/step, step, V, M).transpose((0,1,3,2,4)).reshape(C, T/step, V, M*step)
...
def random_move(
    ...
    for i in range(num_node-1):
        ...
        t_y[node[i]:node[i+1]] = np.linspac(
            T_y[i], T_y[i+1], node[i+1] - node[i]
        )
        
    theta = np.array([
        [np.cos(a)*s, -np.sin(a)*s],
...
def openpose_match(data_numpy):
    ...
    # sequence of distance between frame 1 and 2 (shape: T-1, M, M)
    ...
    data_numpy = new_data_numpy[:, :, :, rank]
    
    return data_numpy

def top_k_by_category(labe, score, top_k):
